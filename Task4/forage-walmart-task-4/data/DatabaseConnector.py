        ...
        for row_index, row in enumerate(csv_reader_0):
            ...
            if row_index > 0:
                ...
                self.insert_shipment(
                    product_name, product_quantity, origin, destination)
...
# Give an indication of progress
                count += 1
                print(
                    f"Inserted product {count} from shipping_data_1 and shipping_data_2")


def insert_product_if_it_does_not_already_exist(self, product_name):
    """
    Insert a product into the database if it does not already exist.
    """
...
def insert_shipment(self, product_name, product_quantity, origin, destination):
    """
    Insert a shipment into the database.
    """
    # Check if the shipment already exists in the database
    query = "SELECT * FROM shipments WHERE product_name = ? AND origin = ? AND destination = ?"
    self.cursor.execute(query, (product_name, origin, destination))
    result = self.cursor.fetchone()

    # If the shipment does not exist, insert it into the database
    if result is None:
        query = "INSERT INTO shipments (product_name, product_quantity, origin, destination) VALUES (?, ?, ?, ?)"
        self.cursor.execute(
            query, (product_name, product_quantity, origin, destination))
        self.connection.commit()
